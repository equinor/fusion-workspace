<template>
	<a :href="npmUrl" target="npm">
		<img alt="GitHub package.json version (subfolder of monorepo)" :src="url" />
	</a>
</template>
...
export default {
	...
	computed: {
		url() {
			const url = new URL(`https://img.shields.io/github/package-json/v/equinor/fusion-workspace`);
		...
		packageName() {
			return this.package || `@equinor/${this.module}`;
		},
		npmUrl() {
