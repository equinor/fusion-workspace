...
export class FilterController<TData> {
	...
	private unsubOnFilterDataChange = (id: string) => {
		this.onFilteredDataChangedCallbacks = this.onFilteredDataChangedCallbacks.filter((s) => s.id !== id);
	};
	...
	filter = () => {
		...
		this.setFilteredData(
			this.data.filter((item) => doesItemPassFilter(item, this.filterState, this.valueFormatters))
		);
