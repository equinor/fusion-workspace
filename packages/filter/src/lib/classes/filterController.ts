...
export class FilterController<TData> {
  ...
  onFilteredDataChangedCallbacks: Callback<
    OnFilterDataChangeCallback<TData>
  >[] = [];
  onFilterStateChangedCallbacks: Callback<
    OnFilterStateChangedCallback<TData>
  >[] = [];
   ...
   */
  onFilterStateChange = (
    cb: OnFilterStateChangedCallback<TData>
  ): OnCallbackSet =>
    registerCallback(
      cb,
      this.onFilterStateChangedCallbacks,
      this.unsubOnFilterStateChange
    );

  private unsubOnFilterStateChange = (id: string) => {
    this.onFilterStateChangedCallbacks =
      this.onFilterStateChangedCallbacks.filter((s) => s.id !== id);
  };
  ...
  onFilterDataChange = (cb: OnFilterDataChangeCallback<TData>): OnCallbackSet =>
    registerCallback(
      ...
      this.unsubOnFilterDataChange
    );

  private unsubOnFilterDataChange = (id: string) => {
    this.onFilteredDataChangedCallbacks =
      this.onFilteredDataChangedCallbacks.filter((s) => s.id !== id);
  };
  ...
  getFilterItemCountsForGroup = (groupName: string): FilterItemCount[] => {
    const filterGroup = this.allFilterValues.find(
      ({ name }) => name === groupName
    );
  ...
  getCountForFilterValue = (
    ...
    const valueFormatter =
      valueFormatterFunc ??
      this.valueFormatters.find(({ name }) => name === filterGroup.name)
        ?.valueFormatter;
    if (!valueFormatter) return -1;

    const uncheckedValues = filterGroup.values.filter(
      (value) => value !== filterItem
    );

    return this.filteredData.reduce((count, val) => {
      return doesItemPassCriteria(uncheckedValues, valueFormatter(val))
        ? count + 1
        : count;
    }, 0);
  };
  ...
  createFilterValues = () => {
    this.allFilterValues = generateFilterValues(
  ...
  markAllValuesActive = (groupName: string) => {
    ...
    if (!group) return;
    this.setFilterState(
      this.filterState.filter(({ name }) => name !== groupName)
    );
    this.filter();
  };
