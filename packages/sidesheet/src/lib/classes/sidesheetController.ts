...
export class SidesheetController<TData, TContext> {
  ...
  private onSidesheetStateChangedCallbacks: Callback<
    SidesheetChangedCallback<TData>
  >[] = [];
  ...
  onSidesheetStateChanged = (
    cb: SidesheetChangedCallback<TData>
  ): OnCallbackSet => {
  ...
  setSidesheetState = (state: SidesheetState, preventCallbacks?: boolean) => {
    ...
    if (!preventCallbacks) {
      this.onSidesheetStateChangedCallbacks.forEach(({ callback }) =>
        callback(state, this)
      );
    }
  };
