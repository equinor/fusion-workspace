...
export const NodeStoreProvider = ({ children, contextMerge }: NodeStoreProviderProps) => {
	const existingStore = useContext(NodeStoreContext);

	const store = useRef(contextMerge && existingStore ? existingStore : createStore());
	//TODO: Maybe just render children directly if parent store is accessible?
	//Probably doesnt matter
	return <NodeStoreContext.Provider value={store.current}>{children}</NodeStoreContext.Provider>;
};
