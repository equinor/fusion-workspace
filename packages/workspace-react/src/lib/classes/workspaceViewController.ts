...
export class WorkspaceViewController<TTabNames extends string, TError> {
    ...
    setActiveTab = (tabName: TTabNames) => {
        ...
        this.onActiveTabChangedCallbacks.forEach(({ callback }) => callback(tabName, oldTabName, this));
    };
    ...
    setIsFilterOpen = (isOpen: boolean) => {
        ...
        this.onFilterOpenOrClosedCallbacks.forEach(({ callback }) => callback(isOpen, this));
    };
    ...
    setIsLoading = (isLoading: boolean) => {
        ...
        this.onIsLoadingChangedCallbacks.forEach(({ callback }) => callback(isLoading, this));
    };
    ...
    private removeOnActiveTabChangedCallback = (id: string) =>
        (this.onActiveTabChangedCallbacks = this.onActiveTabChangedCallbacks.filter((s) => s.id !== id));
    private removeOnIsLoadingCallback = (id: string) =>
        (this.onIsLoadingChangedCallbacks = this.onIsLoadingChangedCallbacks.filter((s) => s.id !== id));
    private removeOnFilterOpenOrClosedCallback = (id: string) =>
        (this.onFilterOpenOrClosedCallbacks = this.onFilterOpenOrClosedCallbacks.filter((s) => s.id !== id));
}
