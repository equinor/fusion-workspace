...
> {
  ...
  private onClickCallbacks: Callback<
    OnClickCallback<TData, TControllers, TOnClick, TError, TContext>
  >[] = [];

  private onErrorCallbacks: Callback<
  ...
  addMiddleware = (middleware: MiddlewareConfigFunction<this>) =>
    middleware(this);

  setContext = (context: ContextCallbackSetter<TContext | undefined>) =>
    (this.context = context(this.context));

  setData = (data: TData[], preventCallbacks?: boolean) =>
    (this.data = data) &&
    !preventCallbacks &&
    this.notifyOnDataChangedCallbacks();

  setFilteredData = (data: TData[], preventCallbacks?: boolean) =>
    (this.filteredData = data) &&
    !preventCallbacks &&
    this.notifyOnFilteredDataChangedCallbacks();
  ...
  onDataChanged = (
    ...
    this.addCallback(
      this.onDataChangedCallbacks,
      makeCallback(callback),
      this.removeOnDataChangedCallback
    );
  ...
  onClick = (
    ...
    this.addCallback(
      ...
      makeCallback(callback),
      this.removeOnClickCallback
    );

  onError = (
    callback: WorkspaceErrorCallback<
      TData,
      TControllers,
      TOnClick,
      TError,
  ...
  click = (ev: TOnClick) =>
    this.onClickCallbacks.forEach(({ callback }) => callback(ev, this));
  ...
  private removeOnFilteredDataChangedCallback = (id: string) => {
    this.onFilteredDataChangedCallbacks =
      this.onFilteredDataChangedCallbacks.filter((cb) => cb.id !== id);
  };
